...
# -*- coding: utf-8 -*-
"""
Created on Tue May 14 12:22:36 2024
...
def get_tickers_price(symbols,_start_date,_end_date):
    ...
    if os.path.isfile(STOCK_DATA_FILE):
        ...
        print ('max(df.index) >= _end_date',ENDDATE_minus_1 >= _end_date, max(df.index),ENDDATE_minus_1, _end_date)
        if set(symbols).issubset(df.columns) and\
               STARTDATE_minus_1 <= _start_date and \
                ENDDATE_minus_1 >= _end_date:
            ...
            df.to_csv('./data/new_stock_data.csv')
            return df.loc[(df.index<=_end_date)]
            ...
            for s in common_symbols:
                ...
                if notnan(shortMAratio[s].iloc[target_index]) and \
                    ...
                    # if it is our target symbols give it a special color RED
                        ...
                        ax.scatter(shortMAratio[s].iloc[target_index],longMAratio[s].iloc[target_index], alpha=0.3, color='gray', zorder=50) #cmap=plt.cm.Spectral)
...
if __name__=='__main__':
    ...
    ENDDATE = targetdate = '2020-03-16'
    # ENDDATE = targetdate = '1987-10-19'
    PLOTDAYS = 8
    SHORTMA = 50
    LONGMA = 200
    plotpercentile = 100
    ...
    fig, axes = plt.subplots(dpi=100, nrows=num_rows, ncols=num_cols, figsize=(num_cols * 6, num_rows * 4), sharex=False,sharey=False)
     
    ani = FuncAnimation(fig, plot_chart , fargs=[targetdate, axes], init_func=lambda: None, frames=range(PLOTDAYS))
    ani.save('./img/'+title+'_(MA='+str(SHORTMA)+'_'+str(LONGMA)+')_'+STARTDATE+'_'+ENDDATE+'_'+'.gif', writer='imagemagick', fps=3)
